...
export const generateQuestions = async (file, numQuestions = 5, difficulty = 'medium', mode = 'mcq', youtubeUrl = null) => {
...
export const getAdvancedDictionary = async (word) => {
    const response = await fetch(`${API_URL}/dictionary/advanced/${encodeURIComponent(word)}`);
    if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.detail || 'Failed to fetch dictionary data');
    }
    return response.json();
};
