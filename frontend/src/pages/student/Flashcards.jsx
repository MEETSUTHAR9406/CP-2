...
import { ChevronLeft, ChevronRight, Bookmark, Sparkles, RotateCcw, UploadCloud } from 'lucide-react';
...
const Flashcards = () => {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      // Generate QA type questions for flashcards
      const data = await generateQuestions(file, 10, 'medium', 'qa');
  ...
  const currentCard = cards[currentIndex] || MOCK_CARDS[0];
  ...
  return (
    <div className="max-w-3xl mx-auto flex flex-col items-center justify-center min-h-[80vh]">
      <div className="w-full text-center mb-8">
        ...
        <div className="mt-6 max-w-md mx-auto relative group">
           <label className="flex items-center justify-center gap-3 p-4 bg-white border-2 border-dashed border-gray-200 hover:border-indigo-400 rounded-xl cursor-pointer transition-colors shadow-sm text-gray-600 hover:text-indigo-600 font-medium">
             <UploadCloud size={24} className={clsx(isGenerating && "animate-bounce text-indigo-500")} />
             {isGenerating ? "Generating New Cards..." : "Upload PDF to Generate Cards"}
             <input type="file" accept=".pdf,.txt" className="hidden" onChange={handleFileUpload} disabled={isGenerating} />
           </label>
        </div>
      </div>

      {cards.length === 0 && !isGenerating ? (
         <div className="text-center py-20 text-gray-500">No flashcards available yet. Upload a document above to get started!</div>
      ) : (
      <>
